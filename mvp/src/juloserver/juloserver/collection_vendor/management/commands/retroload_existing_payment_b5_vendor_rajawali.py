...
class Command(BaseCommand):
    help = 'retroload data b5 existing on vendor rajawali insert to collection assignment vendor'

    def handle(self, *args, **options):
        csv_file_name = 'misc_files/csv/payments_assign_to_vendor_rajawali.csv'
        ...
        try:
            ...
            for row in rows:
                if row['application_xid'] == '5103140130':
                    continue
                assign_date = parse_datetime(row['assign_date'])
                exclude_date = datetime.strptime('2020-06-26', '%Y-%m-%d')
                if assign_date.date() == exclude_date.date():
                    continue
