...
CREATE OR REPLACE VIEW ops.fdc_data_as_view
AS SELECT row_number() OVER () AS fdc_data_as_view_id,
   ...
   FROM ops.customer c
     ...
     LEFT JOIN ( SELECT application_history.application_id,
          ...
          GROUP BY application_history.application_id) sphp_ready ON sphp_ready.application_id = a.application_id
     LEFT JOIN ( SELECT payment.loan_id,
           ...
           FROM ops.payment
          GROUP BY payment.loan_id) ph ON ph.loan_id = l.loan_id
     LEFT JOIN ( SELECT payment.loan_id,
            min(
                CASE
           ...
           FROM ops.payment
          WHERE payment.payment_status_code < 330 AND payment.is_restructured = false
          GROUP BY payment.loan_id) p ON p.loan_id = l.loan_id
     LEFT JOIN ( SELECT DISTINCT payment_history.loan_id
           FROM ops.payment_history
          WHERE payment_history.cdate > (timezone('Asia/Jakarta'::text, now())::date - 1) AND payment_history.cdate < timezone('Asia/Jakarta'::text, now())::date) lc ON lc.loan_id = l.loan_id
  WHERE a.application_status_code = 180 AND l.loan_status_code <> 210 AND lower(a.email::text) !~~ '%julofinance%'::text
UNION ALL
 SELECT row_number() OVER () AS fdc_data_as_view_id,
    ...
    to_char(x212."time", 'YYYYMMDD'::text) AS tgl_perjanjian_borrower,
        CASE
            WHEN l.fund_transfer_ts > ph.last_due_date THEN to_char((timezone('Asia/Jakarta'::text, x212."time")::date + 1)::timestamp with time zone, 'YYYYMMDD'::text)
    ...
    timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.oldest_due_date, timezone('Asia/Jakarta'::text, now())::date) AS status_pinjaman_max_dpd,
        CASE
            ...
            WHEN l.loan_status_code = 250 THEN 'L'::text
            WHEN l.loan_status_code = ANY (ARRAY[237, 260]) THEN 'W'::text
            WHEN l.loan_status_code = ANY (ARRAY[210, 216, 217]) THEN 'D'::text
            ELSE NULL::text
        END AS status_pinjaman
   FROM ops.customer c
     ...
     JOIN ( SELECT loan_history.loan_id,
          ...
          GROUP BY loan_history.loan_id) x212 ON x212.loan_id = l.loan_id
     ...
     JOIN ( SELECT loan_history.loan_id,
          ...
          GROUP BY loan_history.loan_id) x220 ON x220.loan_id = l.loan_id
     LEFT JOIN ( SELECT DISTINCT payment_history.loan_id
           FROM ops.payment_history
          WHERE payment_history.cdate > (timezone('Asia/Jakarta'::text, now())::date - 1) AND payment_history.cdate < timezone('Asia/Jakarta'::text, now())::date) lc ON lc.loan_id = l.loan_id
  WHERE a.application_status_code = 190 AND l.loan_status_code not in (210, 241) AND lower(a.email::text) !~~ '%julofinance%'::text;

-- Permissions

ALTER TABLE ops.fdc_data_as_view OWNER TO postgres;
GRANT ALL ON TABLE ops.fdc_data_as_view TO postgres;
GRANT SELECT ON TABLE ops.fdc_data_as_view TO readonly_users;
GRANT SELECT ON TABLE ops.fdc_data_as_view TO analytics_users;
GRANT ALL ON TABLE ops.fdc_data_as_view TO admin_users;
GRANT ALL ON TABLE ops.fdc_data_as_view TO ops_server;
