    ...
    def handle(self, *args, **options):
        ...
        for file in files:
            ...
            for index, row in enumerate(listemail):
                ...
                if row_splited[1].strip() != "no":
                    email = row_splited[2].strip()
                    name = row_splited[3].strip()
                    try:
                        row_splited[4]
                        if row_splited[4].strip() == "Sent":
                            processed_row =True
                        else:
                            processed_row = False
                    except:
                    ...
                    if not processed_row:
                        ...
                        try:
                            status, headers, subject, content = julo_email_client.email_julo_promo_blast(email, name)
                        except EmailNotSent:
                            bad_emails.append(email)
                            bad_email_count += 1
                            self.stdout.write(
                            self.style.ERROR("Skiping email : " + email + " format bermasalah"))
                            if len(row_splited) == 4:
                                row_splited.append("Failed")
                            else:
                                row_splited[4] = "Failed"
                        ...
                        if status == 202:
                            ...
                            bad_email_count += 1
                            self.stdout.write(
                                self.style.ERROR("Skiping email : " + email + " aplikasi tidak ditemukan"))
                            if len(row_splited) == 4:
                                row_splited.append("Failed")
                            else:
                                row_splited[4] = "Failed"
