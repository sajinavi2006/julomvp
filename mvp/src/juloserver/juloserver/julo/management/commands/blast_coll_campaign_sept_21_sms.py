...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        folder = '../../email_blast/blast_coll_campaign_sept_21/sms/'
        ...
        for file in files:
            ...
            if len(listphonenumber[0].split(",")) == 18:
            ...
            for index, row in enumerate(listphonenumber):
                ...
                if row_splited[0].strip() != "variant":
                    ...
                    phonenumber = row_splited[8].strip()
                    phonenumber2 = row_splited[9].strip()
                    if variant == "Control: No Promo":
                        continue
                    if variant == 'Test: HP Samsung':
                        text = "Pelanggan JULO! Menangkan Undian HP SAMSUNG juta dgn membayar Pinjaman 2 HARI SEBELUM tanggal jatuh tempo. AYO bayar skrng! bit.ly/juloapp"
                    elif variant == 'Test: 2 juta':
                        text = "Pelanggan JULO! Menangkan Undian UANG TUNAI Rp 2 juta dgn membayar Pinjaman 2 HARI SEBELUM tanggal jatuh tempo. AYO bayar skrng! bit.ly/juloapp"
                    elif variant == 'Test: 5 juta':
                        text = "Pelanggan JULO! Menangkan Undian UANG TUNAI Rp 5 juta dgn membayar Pinjaman 2 HARI SEBELUM tanggal jatuh tempo. AYO bayar skrng! bit.ly/juloapp"
                    ...
                    if not processed_row:
                        ...
                        if response['status'] != '0':
                            ...
                            bad_number_count += 1
                            self.stdout.write(
                                self.style.ERROR("Skiping Number : " + phonenumber + "error: " + json.dumps(response)))
                            if len(row_splited) == 18:
                                row_splited.append("Failed")
                            else:
                                row_splited[18] = "Failed"
