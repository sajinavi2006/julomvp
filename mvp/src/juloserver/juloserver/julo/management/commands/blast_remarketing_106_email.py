...
class Command(BaseCommand):
    help = 'blast email for 106 remarketing'
    ...
    def handle(self, *args, **options):
        ...
        if api_key == "--------------------------------sendgrid_API_KEY----------------------":
            ...
            return
        folder = '../../email_blast/remarketing_106/'
        ...
        for file in files:
            ...
            for index, row in enumerate(listemail):
                ...
                if row_splited[0].strip() != "customer_id":
                    email = row_splited[3].strip()
                    fullname = row_splited[1].strip()
                    ...
                    if not processed_row and not skip_sent:
                        ...
                        try:
                            status, headers, subject, content = julo_email_client.remarketing_106_blast(email, gender, fullname)
                        except EmailNotSent:
