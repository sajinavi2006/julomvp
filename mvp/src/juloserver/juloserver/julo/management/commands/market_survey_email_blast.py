...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if api_key == "--------------------------------sendgrid_API_KEY----------------------":
            ...
            return
        folder = '../../email_blast/market_survey_email/'
            ...
            for index, row in enumerate(listemail):
                ...
                if row_splited[0].strip() != "no":
                    ...
                    if not processed_row:
                        ...
                        try:
                            status, headers, subject, content = julo_email_client.email_market_survey_blast(email)
                        except EmailNotSent:
                            bad_emails.append(email)
                            bad_email_count += 1
                            self.stdout.write(
                            self.style.ERROR("Skiping email : " + email + " format bermasalah"))
                            if len(row_splited) == 2:
                                row_splited.append("Failed")
                            else:
                                row_splited[2] = "Failed"
