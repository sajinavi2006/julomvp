...
class Command(BaseCommand):
    help = 'insert data sell off october campaign'

    def handle(self, *args, **options):
        csv_file_name = '../../email_blast/sell_off_oct_campaign/sell_off_oct_campaign.csv'
        try:
            ...
            for row in rows:
                payments = Payment.objects.filter(
                    ...
                    loan__application=row['application_id'])
                ...
                if payments:
                    for payment in payments:
                        WaivePromo.objects.update_or_create(
                            ...
                            promo_event_type=WaiveCampaignConst.SELL_OFF_OCT)

            self.stdout.write(self.style.SUCCESS(
                'Successfully load %s data to waive promo table' % WaiveCampaignConst.SELL_OFF_OCT)
            )
        except IOError:
