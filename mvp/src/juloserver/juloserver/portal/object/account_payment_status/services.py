...
def check_account_payment_first_installment_btn_active(account_payment):
    ...
    if not account_payment.is_paid and oldest_unpaid_payment and \
            oldest_unpaid_payment.status_id in [PaymentStatusCodes.PAYMENT_NOT_DUE,
...
def find_phone_number_from_application_table(qs, phone_number):
    ...
    data = {}
    list_of_id = []
    fields = ['additional_contact_1_number','additional_contact_2_number', 'company_phone_number',
              'close_kin_mobile_phone', 'kin_mobile_phone', 'mobile_phone_2',
              'landlord_mobile_phone', 'new_mobile_phone', 'spouse_mobile_phone']

    data['additional_contact_1_number'] = qs.filter(account__application__additional_contact_1_number=phone_number)
    data['additional_contact_2_number'] = qs.filter(account__application__additional_contact_2_number=phone_number)
    data['company_phone_number'] = qs.filter(account__application__company_phone_number=phone_number)

    data['close_kin_mobile_phone'] = qs.filter(account__application__close_kin_mobile_phone=phone_number)
    ...
    data['landlord_mobile_phone'] = qs.filter(account__application__landlord_mobile_phone=phone_number)
