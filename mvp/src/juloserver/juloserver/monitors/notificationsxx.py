...
def get_slack_bot_client():
    return SlackClient("xoxb-276848641921-viGpSf3cWbcIHYNfTJkeq0Yo")
...
def notify_dana_collection_each_process(task_name):
    title = 'Dana Collection Process Tracking'

    text = "=== *%s* ===\n" % title + timezone.localtime(timezone.now()).strftime(
        "*%A*, *%Y-%m-%d | %H:%M*"
    )
    attachment = "This `{}` has been FINISHED :rocket:".format(task_name)
...
def notify_successful_lion_parcel_data_upload():
    message_to_slack = (
        "<!here>"
        "\nHEADERS: LION PARCEL DELIVERY DETAILS"
        "\nBODY: Lion parcel data delivery details added successfully in {} env".format(settings.ENVIRONMENT)
    )
    slack_bot_client = get_slack_bot_client()
    slack_bot_client.api_call(
        "chat.postMessage",
