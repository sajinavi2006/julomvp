# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2023-11-30 13:58
...
def create_mba_consult_lender(apps, schema_editor):
    ...
    if not partner:
        ...
        lender = LenderCurrent.objects.create(
            ...
            poc_email=poc_email,
            poc_name=poc_name,
            poc_position="Chief Risk Officer",
            poc_phone=poc_phone,
            source_of_fund=source_of_fund,
...
def update_list_lender_info_fs_for_mba_consult_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
        feature_name=FeatureNameConst.LIST_LENDER_INFO
    ).last()
    feature_setting.parameters['lenders']['mba_consult_lender'] = {
        'address': '160 Robinson Road, #24-09, Singapore (068914)',
        'license_no': '202312345Z',
        'poc_name': 'Aleksei Terskii',
        'poc_position': 'Chief Risk Officer',
        'signature': '',
    }
...
def create_new_certificate_for_mba_consult_lender(apps, schema_editor):
    mba_consult_lender = LenderCurrent.objects.get_or_none(lender_name='mba_consult_lender')
    if mba_consult_lender:
        generate_lender_signature(lender=mba_consult_lender, signature_key='lender')


def update_signature_key_configuration_for_mba_consult_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    mba_consult_lender = LenderCurrent.objects.get_or_none(lender_name='mba_consult_lender')
    if mba_consult_lender:
        feature_setting.parameters['users'][str(mba_consult_lender.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_mba_consult_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_mba_consult_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_mba_consult_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_mba_consult_lender, migrations.RunPython.noop
        ),
    ]
