# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2022-07-08 08:28
from __future__ import unicode_literals

from django.db import migrations
from juloserver.julo.models import (ProductLine,
                                    LoanPurpose,
                                    ProductLineLoanPurpose)


def add_new_loan_purpose_risky_for_j1(apps, schema_editor):
    """
    Set Product line code for J1
    product_line_code = 1
    """

    pd_line_code = 1
    new_loan_purpose = "Investasi saham / Forex / Crypto"
    product = ProductLine.objects.get_or_none(pk=pd_line_code)
    if product:
        # checking data
        loan_purpose = LoanPurpose.objects.get_or_none(purpose=new_loan_purpose)
        if loan_purpose:
            # check relation
            related = ProductLineLoanPurpose.objects.get_or_none(product_line=product,
                                                                 loan_purpose=loan_purpose)
            if not related:
                ProductLineLoanPurpose.objects.create(product_line=product,
                                                      loan_purpose=loan_purpose)


class Migration(migrations.Migration):

    dependencies = [
    ]

    operations = [
        migrations.RunPython(add_new_loan_purpose_risky_for_j1, migrations.RunPython.noop)
    ]
