# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2024-10-23 20:00
...
USERNAME = 'smf_channeling'
ADDRESS = (
    'Gedung Cyber 2 Tower, Lantai 29. Jl. H. R. Rasuna Said, X-5, '
    'Kuningan Timur, Setiabudi, Kota Adm. Jakarta Selatan, DKI Jakarta, 12950'
)
LICENSE_NUMBER = 'KEP-102/D.05/2015'
POC_NAME = 'Rizky Sonia Pradja'
POC_POSITION = 'Perwakilan Lender'
SIGNATURE_KEY = 'oldlender'


def create_smf_channeling_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="bss_channeling").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    company_name = 'PT Saison Modern Finance'
    business_type = 'Perusahaan Pembiayaan'
    poc_email = 'rizky@julo.co.id'
    poc_phone = '+6281286788226'
    ...
    if not partner:
        ...
        lender = LenderCurrent.objects.create(
            ...
            is_pre_fund_channeling_flow=True,
...
def update_list_lender_info_fs_for_smf_channeling_lender(apps, schema_editor):
...
def create_new_certificate_for_smf_channeling_lender(apps, schema_editor):
    smf_channeling_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if smf_channeling_lender:
        generate_lender_signature(lender=smf_channeling_lender, signature_key=SIGNATURE_KEY)


def update_signature_key_configuration_for_smf_channeling_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    smf_channeling_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if smf_channeling_lender:
        feature_setting.parameters['users'][str(smf_channeling_lender.user.id)] = SIGNATURE_KEY
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_smf_channeling_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_smf_channeling_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_smf_channeling_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_smf_channeling_lender, migrations.RunPython.noop
        ),
    ]
