# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2024-05-02 16:13
...
USERNAME = 'bpr_xen'
ADDRESS = 'Jalan Margonda Raya No. 99c Kelurahan Kemiri Muka Kecamatan Beiji, Kota Depok'
LICENSE_NUMBER = '0220002761'
POC_NAME = 'David Purba'
POC_POSITION = 'Direktur Utama'
SIGNATURE_KEY = 'lender'


def create_bpr_xen_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="blue_finc_lender").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    company_name = 'BPR XEN'
    business_type = 'Banking'
    poc_email = 'david@bprxen.co'
    poc_phone = '+6281181200612'
...
def update_list_lender_info_fs_for_bpr_xen_lender(apps, schema_editor):
...
def create_new_certificate_for_bpr_xen_lender(apps, schema_editor):
    bpr_xen_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if bpr_xen_lender:
        generate_lender_signature(lender=bpr_xen_lender, signature_key=SIGNATURE_KEY)


def update_signature_key_configuration_for_bpr_xen_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    bpr_xen_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if bpr_xen_lender:
        feature_setting.parameters['users'][str(bpr_xen_lender.user.id)] = SIGNATURE_KEY
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_bpr_xen_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_bpr_xen_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(create_new_certificate_for_bpr_xen_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_signature_key_configuration_for_bpr_xen_lender, migrations.RunPython.noop
        ),
    ]
