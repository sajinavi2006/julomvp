...
USERNAME = 'dbs_channeling'
ADDRESS = 'DBS BANK TOWER LANTAI 32 – 37, CIPUTRA WORLD 1, JL. PROF. DR. SATRIO KAV. 3 & 5, KARET KUNINGAN, SETIABUDI, JAKARTA SELATAN 12940'
LICENSE_NUMBER = '1294002412733'
POC_NAME = 'Willy Lawy'
POC_POSITION = 'Head of Ecosystem Lending'
SIGNATURE_KEY = 'lender'


def create_dbs_channeling_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="permata_channeling").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    company_name = 'PT BANK DBS INDONESIA'
    business_type = 'Bank'
    poc_email = 'willylawy@dbs.com'
    poc_phone = '+62818167471'
...
def update_list_lender_info_fs_for_dbs_channeling_lender(apps, schema_editor):
...
def create_new_certificate_for_dbs_channeling_lender(apps, schema_editor):
    dbs_channeling_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if dbs_channeling_lender:
        generate_lender_signature(lender=dbs_channeling_lender, signature_key=SIGNATURE_KEY)


def update_signature_key_configuration_for_dbs_channeling_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    dbs_channeling_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if dbs_channeling_lender:
        feature_setting.parameters['users'][str(dbs_channeling_lender.user.id)] = SIGNATURE_KEY
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_dbs_channeling_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_dbs_channeling_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_dbs_channeling_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_dbs_channeling_lender, migrations.RunPython.noop
        ),
    ]
