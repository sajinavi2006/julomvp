# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2024-05-03 04:25
...
USERNAME = 'visionet_2'
ADDRESS = 'South Quarter, Tower B, Lantai 20, Unit A, B, C & D, Jl. R.A. Kartini Kav. 8, Jakarta Selatan 12430, Daerah Khusus Ibukota Jakarta 12920'
LICENSE_NUMBER = '9120306561159'
POC_NAME = 'Dion Soetadi'
POC_POSITION = 'Director'


def add_visionet_indonesia_2_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="blue_finc_lender").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    company_name = 'PT Visionet Internasional'
    business_type = 'Penyedia Jasa Pembayaran (KBLI66641) serta Portal Web dan Penyedia Platform Digital (KBLI 63122)'
    poc_email = 'dion.soetadi@ovo.id'
    poc_phone = ''
    pks_number = '1.JTF.201707'
    source_of_fund = 'Internal Funding'
...
def update_list_lender_info_fs_for_lender_visionet_indonesia_2(apps, schema_editor):
...
def create_new_certificate_for_lender_visionet_indonesia_2(apps, schema_editor):
    v2_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if v2_lender:
        generate_lender_signature(lender=v2_lender, signature_key='lender')


def update_signature_key_configuration_for_lender_visionet_indonesia_2(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    v2_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if v2_lender:
        feature_setting.parameters['users'][str(v2_lender.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            code=add_visionet_indonesia_2_lender,
            reverse_code=migrations.RunPython.noop,
        ),
        migrations.RunPython(
            code=update_list_lender_info_fs_for_lender_visionet_indonesia_2,
            reverse_code=migrations.RunPython.noop,
        ),
        migrations.RunPython(
            code=create_new_certificate_for_lender_visionet_indonesia_2,
            reverse_code=migrations.RunPython.noop,
        ),
        migrations.RunPython(
            code=update_signature_key_configuration_for_lender_visionet_indonesia_2,
            reverse_code=migrations.RunPython.noop,
        ),
    ]
