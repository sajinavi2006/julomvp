# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2024-04-05 09:01
...
USERNAME = 'pepper_global'
ADDRESS = 'Treasury Tower Lantai 31, Jalan Jend Sudirman Kav 52-53, District 8 SCBD Jakarta Selatan 12190 qq. PT Pepper Services Indonesia'
LICENSE_NUMBER = '0204240048271'
POC_NAME = 'Danang Ari Wibowo'
POC_POSITION = 'Direktur'


def create_sell_off_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="blue_finc_lender").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    company_name = 'PT Kinerja Inovasi Wira Indonesia'
    business_type = 'Perusahaan Umum'
    poc_email = 'kinerja.kiwi@gmail.com'
    poc_phone = '+6281289715688'
...
def update_list_lender_info_fs_for_sell_off_lender(apps, schema_editor):
...
def create_new_certificate_for_sell_off_lender(apps, schema_editor):
    sell_off_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if sell_off_lender:
        generate_lender_signature(lender=sell_off_lender, signature_key='lender')


def update_signature_key_configuration_for_sell_off_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    sell_off_lender = LenderCurrent.objects.get_or_none(lender_name=USERNAME)
    if sell_off_lender:
        feature_setting.parameters['users'][str(sell_off_lender.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_sell_off_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_sell_off_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(create_new_certificate_for_sell_off_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_signature_key_configuration_for_sell_off_lender, migrations.RunPython.noop
        ),
    ]
