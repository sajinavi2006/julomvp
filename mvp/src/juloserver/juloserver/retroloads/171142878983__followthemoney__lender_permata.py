# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2024-03-26 04:53
...
def create_permata_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="bss_channeling").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    username = 'permata_channeling'
    company_name = 'PT Bank Permata Tbk'
    address = 'Gedung World Trade Center II, Kompleks Metropolitan, Jalan Jendral Sudirman Kav. 29-31, Jakarta 12930'
    business_type = 'Banking'
    poc_position = 'Kuasa Pemberi Dana'
    poc_name = 'Rizky Sonia Pradja'
    poc_email = 'rizky@julo.co.id'
    poc_phone = '+6281111111111'  # dummy, will update via CRM later
    pks_number = '1.JTF.201707'
    source_of_fund = 'Other'
    license_number = '1111111111111'  # dummy, will update via CRM later
...
def update_list_lender_info_fs_for_permata_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
        feature_name=FeatureNameConst.LIST_LENDER_INFO
    ).last()
    feature_setting.parameters['lenders']['permata_channeling'] = {
        'address': 'Gedung World Trade Center II, Kompleks Metropolitan, Jalan Jendral Sudirman Kav. 29-31, Jakarta 12930',
        'license_no': '1111111111111',  # dummy, will update via CRM later
        'poc_name': 'Rizky Sonia Pradja',
        'poc_position': 'Kuasa Pemberi Dana',
        'signature': '',
    }
...
def create_new_certificate_for_permata_lender(apps, schema_editor):
    permata_channeling = LenderCurrent.objects.get_or_none(lender_name='permata_channeling')
    if permata_channeling:
        generate_lender_signature(lender=permata_channeling, signature_key='lender')


def update_signature_key_configuration_for_permata_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    permata_channeling = LenderCurrent.objects.get_or_none(lender_name='permata_channeling')
    if permata_channeling:
        feature_setting.parameters['users'][str(permata_channeling.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            create_permata_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_list_lender_info_fs_for_permata_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_permata_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_permata_lender, migrations.RunPython.noop
        ),
    ]
