# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2023-11-20 10:52
# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2023-07-03 07:01
...
def create_legend_capital_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="blue_finc_lender").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    username = 'legend_capital_lender'
    company_name = 'Legend Capital Pte, Ltd'
    address = '1 Raffles Place #36-01, One Raffles Place, Singapore 048616'
    business_type = 'Other holding companies'
    poc_name = 'Samantha Fu Minhui'
    poc_email = 'Verity.Priest@maples.com'
    poc_phone = '+6596389296'
...
def update_list_lender_info_fs_for_legend_capital_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
        feature_name=FeatureNameConst.LIST_LENDER_INFO
    ).last()
    feature_setting.parameters['lenders']['legend_capital_lender'] = {
        'address': '1 Raffles Place #36-01, One Raffles Place, Singapore 048616',
        'license_no': '202343390Z',
        'poc_name': 'Samantha Fu Minhui',
        'poc_position': 'Direktur',
        'signature': '',
    }
...
def create_new_certificate_for_legend_capital_lender(apps, schema_editor):
    legend_capital_lender = LenderCurrent.objects.get_or_none(lender_name='legend_capital_lender')
    if legend_capital_lender:
        generate_lender_signature(lender=legend_capital_lender, signature_key='lender')


def update_signature_key_configuration_for_legend_capital_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    legend_capital_lender = LenderCurrent.objects.get_or_none(lender_name='legend_capital_lender')
    if legend_capital_lender:
        feature_setting.parameters['users'][str(legend_capital_lender.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_legend_capital_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_legend_capital_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_legend_capital_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_legend_capital_lender, migrations.RunPython.noop
        ),
    ]
