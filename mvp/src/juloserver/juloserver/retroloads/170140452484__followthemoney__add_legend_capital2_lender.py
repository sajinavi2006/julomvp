# -*- coding: utf-8 -*-
# Generated by Django 1.9.5 on 2023-12-01 04:22
...
def create_legend_capital2_lender(apps, schema_editor):
    base_lender = LenderCurrent.objects.filter(lender_name="blue_finc_lender").last()
    if not base_lender:
        return
    base_partner = base_lender.user.partner

    username = 'legend_capital2_lender'
    company_name = 'Ricky Capital Pte, Ltd'
    address = '1 Raffles Place #36-01, One Raffles Place, Singapore 048616'
    business_type = 'Other holding companies'
    poc_name = 'Melki Sibarani Van Helsing'
    poc_email = 'Melky.Sibarani@Julofinance.com'
    poc_phone = '085716032688'
...
def update_list_lender_info_fs_for_legend_capital2_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
        feature_name=FeatureNameConst.LIST_LENDER_INFO
    ).last()
    feature_setting.parameters['lenders']['legend_capital2_lender'] = {
        'address': '1 Raffles Place #36-01, One Raffles Place, Singapore 048616',
...
def create_new_certificate_for_legend_capital2_lender(apps, schema_editor):
    legend_capital2_lender = LenderCurrent.objects.get_or_none(lender_name='legend_capital2_lender')
    if legend_capital2_lender:
        generate_lender_signature(lender=legend_capital2_lender, signature_key='lender')


def update_signature_key_configuration_for_legend_capital2_lender(apps, schema_editor):
    feature_setting = FeatureSetting.objects.filter(
    ...
    legend_capital2_lender = LenderCurrent.objects.get_or_none(lender_name='legend_capital2_lender')
    if legend_capital2_lender:
        feature_setting.parameters['users'][str(legend_capital2_lender.user.id)] = 'lender'
        feature_setting.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(create_legend_capital2_lender, migrations.RunPython.noop),
        migrations.RunPython(
            update_list_lender_info_fs_for_legend_capital2_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            create_new_certificate_for_legend_capital2_lender, migrations.RunPython.noop
        ),
        migrations.RunPython(
            update_signature_key_configuration_for_legend_capital2_lender, migrations.RunPython.noop
        ),
    ]
