...
class ImageNames(object):
    BG = 'info-card/CARD_BACKGROUND_IMAGE_WEB190.png'
    ICON = 'info-card/document_warning.png'
    BUTTON = 'info-card/R_BUTTON_IMAGE_109.png'
...
def retroload_julo_starter_infocard(_apps, _schema_editor):
    data_to_be_loaded = [
        {
            ...
            'additional_condition': 'HAS_NOT_SIGN_MASTER_AGREEMENT',
            'title': 'Penting! Agar Dapat Lanjutkan Transaksimu',
            'content': 'Yuk, isi serta tandatangani '
            		   'formulir dan dokumennya!',
            'button': ['Isi Sekarang'],
            'button_name': [R_BUTTON],
            'click_to': ['to_additional_form'],
            'template_type': '1',
            'card_number': 1,
            'text_colour': '#ffffff',
            'title_colour': '#ffffff',
            'background_url': ImageNames.BG,
            'additional_images': [ImageNames.ICON],
    ...
    # upload image
    remote_path = 'info-card/document_warning.png'
    upload_file_to_oss(
        settings.OSS_PUBLIC_ASSETS_BUCKET,
        settings.STATICFILES_DIRS[0] + '/images/info_card/document_warning.png',
        remote_path
    )
