...
def apply_query(apps, schema_editor):
    ...
    cursor.execute("""\
    CREATE OR REPLACE VIEW ops.fdc_data_as_view_v5
AS SELECT row_number() OVER () AS fdc_data_as_view_id
    ...
    ,    CASE
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) <= 0 THEN  1
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 1 AND 30 THEN 2
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 31 AND 60 THEN 3
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 61 AND 90 THEN 4
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) > 90 THEN 5
            ELSE NULL::integer
        END AS kualitas_pinjaman
    ,timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date) AS dpd_terakhir
    ,timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.oldest_due_date, timezone('Asia/Jakarta'::text, now())::date) AS dpd_max
    ,    CASE
    ...
    ,case when left(a.mobile_phone_1,1) = '8' then concat(62,a.mobile_phone_1) 
          when left(a.mobile_phone_1,3) = '+62' then concat(62,substring(a.mobile_phone_1 ,4,20))
          when left(a.mobile_phone_1,2) in ('62','o8','6,') then concat(62,substring(a.mobile_phone_1 ,3,20))
          when left(a.mobile_phone_1,2) = '54' then concat(62,substring(a.mobile_phone_1 ,6,20)) 
          when left(a.mobile_phone_1,1) = '0' then concat(62,substring(a.mobile_phone_1 ,2,20)) 
          when left(a.mobile_phone_1,2) in ('ti','Ti','ga') then '62'
       else a.mobile_phone_1 
     end as no_hp
    ,a.email
    ,8 as agunan
    ,NULL::text as tgl_agunan
    ,NULL::text as nama_penjamin
    ,NULL::text as no_agunan
   FROM ops.customer c
...
UNION all
 SELECT row_number() OVER () AS fdc_data_as_view_id
    ...
    ,l.loan_amount::numeric - COALESCE(ph.paid_principal, 0::numeric) AS sisa_pinjaman_berjalan
    ,to_char(timezone('Asia/Jakarta'::text, ph.last_due_date::timestamp with time zone)::date::timestamp with time zone, 'YYYYMMDD'::text) AS tgl_jatuh_tempo_pinjaman
    ,    CASE WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) <= 0 THEN 1
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 1 AND 30 THEN 2 
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 31 AND 60 THEN 3
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) BETWEEN 61 AND 90 THEN 4
            WHEN (timezone('Asia/Jakarta'::text, now())::date - COALESCE(p.earliest_due_date, timezone('Asia/Jakarta'::text, now())::date)) > 90 THEN 5
   ...
   FROM ops.customer c
     ...
     JOIN ops.application a ON a.application_id = l.application_id2 
                            AND (a.application_status_code >= 190 OR product_line_code = 2)
                            AND lower(a.email::text) !~~ '%julofinance%'::text
...
def apply_ownership(apps, schema_editor):
    cursor = connection.cursor()
    cursor.execute("ALTER TABLE ops.fdc_data_as_view_v5 OWNER TO ops_server;")
